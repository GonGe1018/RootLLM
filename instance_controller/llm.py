...
class ResponseFormat(BaseModel):
  ...
  command: CommandFormat | None = Field(None, description="명령 이벤트일 경우 실행할 명령어")
...
class LLM:
  ...
  def generate_response(self, history:list[StepHistory]=[]) -> str: 
    ...
    """
    if len(history) <= 10:
        history_text = "\n".join([f"{entry.model_dump_json()}" for entry in history])
    else:
        ...
        recent_history_text = "\n".join([f"{entry.model_dump_json()}" for entry in recent_history])
